...
func part1() {
	reactions := readPuzzle()
	printReactions(reactions)
}

func readPuzzle() (result map[string]reaction) {
	...
	for scanner.Scan() {
		...
		inputs := parseInputs(parts[0])
		output := parseOutput(parts[1])

		reaction := reaction{output: output, input: inputs}
		result[output.symbol] = reaction
	}
...
func parseInputs(materials string) (result map[string]material) {
	result = make(map[string]material)
	parts := strings.Split(materials, ",")
	for _, p := range parts {
		qty := 0
		symbol := ""
		fmt.Sscanf(strings.TrimSpace(p), "%d %s", &qty, &symbol)
		result[symbol] = material{symbol: symbol, quantity: qty}
	}
	return
}

func parseOutput(material string) (result material) {
	fmt.Sscanf(strings.TrimSpace(material), "%d %s", &result.quantity, &result.symbol)
	return
}

func printReactions(reactions map[string]reaction) {
	for _, r := range reactions {
		for _, v := range r.input {
			fmt.Printf(" %d %s", v.quantity, v.symbol)
		}
		fmt.Print(" => ")
		fmt.Printf(" %d %s\n", r.output.quantity, r.output.symbol)
	}
}
